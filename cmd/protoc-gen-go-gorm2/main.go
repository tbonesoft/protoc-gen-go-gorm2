...
func main() {
	...
	var (
		...
		autoFillNilStringArray = flags.Bool("auto_fill_nil_string_array", true, "auto-fill nil repeated string fields as empty []string in ToPB")
	)

	protogen.Options{
		ParamFunc: func(name string, value string) error {
			if name == "auto_fill_nil_string_array" && value == "" {
				value = "true"
			}
		...
		builder := NewBuilder(gen, *engine, *autoFillNilStringArray)
		builder.Generate()
...
type builder struct {
	gen                    *protogen.Plugin
	dbEngine               DB_ENGINE
	autoFillNilStringArray bool
}

func NewBuilder(gen *protogen.Plugin, dbEngine string, autoFillNilStringArray bool) *builder {
	builder := &builder{
		gen:                    gen,
		dbEngine:               DB_ENGINE_UNSET,
		autoFillNilStringArray: autoFillNilStringArray,
	}
